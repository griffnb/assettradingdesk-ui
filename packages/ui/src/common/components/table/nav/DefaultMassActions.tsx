...
export const DefaultMassActions = observer(function DefaultMassActions<
  T extends object,
>(rawProps: DefaultMassActionsProps<T>) {
  ...
  const count = useMemo(() => {
    if (Object.keys(tableState.checked_row_ids).length == 0 || props.allRows) {
  ...
  const update = async (status: number, name: string) => {
    if (
      window.confirm(`Are you sure you want to make ${count} records ${name} ?`)
    ) {
      const postData: {
