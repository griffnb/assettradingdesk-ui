...
- **Plop**: Code generation and scaffolding
...
- All components should be wrapped in observer()
...
- All components are already installed and available to use in @/ui/shadcn/ui
...
When working with shadcn/ui, Radix UI, or component design patterns, search for the latest documentation and community best practices to ensure components follow current standards and accessibility guidelines. using `#shadcn` or `#context7`

## IMPORTANT
1. Always scaffold first using #ui_code_tools plop generators
2. All shadcn components are already installed so you dont need to install any new ones, they all exist at @/ui/shadcn/ui
3. Use the `#figma` tool if given a figma link
4. If importing code from figma, reduce the amount of tailwind classes that dont do anything to the layout or design, i.e. things like `font-[xxxx]`, `inline-flex` vs `flex flex-row`,  etc. Remove any classes that dont do anything to the layout or design.  Avoid using custom heights and widths, and use proper flow laying out instead
